...
export default function CampaignsList() {
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:3000/api/v1/demo/campaigns');
              ...
              <CardHeader className="pb-3">
                <div className="flex flex-col items-start gap-2 mb-2">
                  <CardTitle className="text-lg text-[#FAFAFA] flex gap-1">
                    <span className="capitalize">{c.name || 'campaign'}</span>
                  </CardTitle>
                  <div className="flex flex-row items-start gap-2 mb-2">
                    <img src={logoBlue} alt="Logo" className="w-6 h-6" />
                    <span className="text-[#EE82DA] uppercase font-bold">{c.token || 'TOKEN'}</span>
                  </div>
                </div>
                <CardDescription className="text-[#FAFAFA]/60">
                  {c.exchange ? `Trading on ${c.exchange}` : 'Trading campaign'} • {c.prize_structure?.replace('_', ' ') || 'Prize structure TBD'}
                </CardDescription>
              </CardHeader>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm number-font text-[#FAFAFA]">
                      {c.prize_pool || '0'} {c.prize_pool_token || c.token || 'tokens'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm number-font text-[#FAFAFA]">
                      {c.start_date_time && c.end_date_time
                        ? `${new Date(c.start_date_time).toLocaleDateString()} - ${new Date(c.end_date_time).toLocaleDateString()}`
                        : 'TBD'
                      }
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-[#FAFAFA]/60">Min Trade</span>
                    <span className="text-sm number-font text-[#FAFAFA]">
                      {c.rules?.min_trade_amount || '0'} {c.token || 'tokens'}
                    </span>
                  </div>
                </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Max Trades/Day</TableHead>
